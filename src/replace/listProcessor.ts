...
import {ProcessDataType} from "./abProcessorInterface"
import type {ABProcessorSpecSimp, ListData, TableData} from "./abProcessorInterface"
 ...
 */
// 新参拟定：1. 最大解析层数、2. 缩进模式、3. 内联语法糖
const process_list2listdata:ABProcessorSpecSimp = {
  id: "list2data",
  name: "列表到列表数据",
  process_param: ProcessDataType.text,
  process_return: ProcessDataType.ListData,
  process: (el, header, content)=>{
    const modeT=false
    const modeG=true
    // if (modeT) return this.ullist2data(content)
    ...
    // 列表文本转列表数据
    let list_itemInfo:ListData = []

    const list_text = content.split("\n")
...
ABProcessManager.getInstance().registerABProcessor(process_list2listdata)



// 标题大纲转列表数据（@todo 正文的level+10，要减掉）
const process_title2listdata:ABProcessorSpecSimp = {
  id: "title2data",
  name: "标题到标题数据",
  process_param: ProcessDataType.text,
  process_return: ProcessDataType.ListData,
...
ABProcessManager.getInstance().registerABProcessor(process_title2listdata)
 ...
 * 第一列的level总为0
 */
const process_ullist2listdata:ABProcessorSpecSimp = {
  id: "ullist2data",
  name: "列表到列表数据",
  process_param: ProcessDataType.text,
  process_return: ProcessDataType.ListData,
