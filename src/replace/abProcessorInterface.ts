/// ab处理器子接口 @warn 暂时不允许扩展，处理器的参数和返回值目前还是使用的手动一个一个来检查的
...
/// ab处理器接口 - 严格版 存储版
export interface ABProcessorSpec<TInput extends keyof ProcessDataType, TOutput extends keyof ProcessDataType>{
  id: string
  name: string
  match: RegExp|string
  default: string|null
  ...
  process: (header:string, content:TInput)=> TOutput
 ...
 */
export interface ABProcessorSpecSimp<TInput extends keyof ProcessDataType, TOutput extends keyof ProcessDataType>{
